...
var ReactCalendarTimeline = function (_Component) {
  ...
  function ReactCalendarTimeline(props) {
    ...
    document.addEventListener('calendarUpdate', _this.forceUpdateCalendar, false);
    return _this;
  }
...
ReactCalendarTimeline.propTypes = {
  ...
  firstZoomAllow: _propTypes2.default.bool, // Adjust calendar zoom at the first seen
  firstZoomRatio: _propTypes2.default.number
};
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.forceUpdateCalendar = function () {
    _this4.resize();
  };
  ...
  this.changeZoom = function (scale) {
    ...
    if (_this4.state.zoomInited === false || _this4.props.firstZoomAllow !== true) {
      ...
      if (_this4.props.firstZoomAllow === true) {
        var defaultMinZoom = 60 * 60 * 1000; // 1 hour
        var defaultMaxZoom = 5 * 365.24 * 86400 * 1000; // 5 years

        newZoom = Math.min(Math.max(Math.round(oldZoom * scale), defaultMinZoom), defaultMaxZoom); // min 1 min, max 20 years
      } else {
        newZoom = Math.min(Math.max(Math.round(oldZoom * scale), minZoom), maxZoom); // min 1 min, max 20 years
      }
